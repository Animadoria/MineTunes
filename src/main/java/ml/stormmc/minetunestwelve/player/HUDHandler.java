...
public class HUDHandler {
    ...
    public void onDrawScreen(RenderGameOverlayEvent.Post event) {
        if(event.getType() == RenderGameOverlayEvent.ElementType.ALL && MTTConfig.hudEnabled) {
            Minecraft mc = Minecraft.getMinecraft();
            int width = event.getResolution().getScaledWidth();
            int height = event.getResolution().getScaledHeight();
            Point coords = getCoords(width, height, MTTConfig.hudRelativeTo, MTTConfig.hudPosX, MTTConfig.hudPosY);

            if(MineTunesTwelve.musicPlayerThread != null && MineTunesTwelve.musicPlayerThread.player != null && MineTunesTwelve.musicPlayerThread.playingMP3 != null) {
                MP3Metadata meta = MineTunesTwelve.musicPlayerThread.playingMP3;

                boolean rightSide = MTTConfig.hudRelativeTo == 0 || MTTConfig.hudRelativeTo == 3;
                boolean topSide = MTTConfig.hudRelativeTo == 0 || MTTConfig.hudRelativeTo == 1;

                String note = MineTunesTwelve.musicPlayerThread.paused ? "\u258E\u258E" : "\u266C";
                int noteWidth = mc.fontRenderer.getStringWidth(note) * 2;
                int noteSpace = 4;

                String time = MP3Metadata.getLengthStr((int) ((double) meta.lengthMs - (double) meta.lengthMs * MineTunesTwelve.musicPlayerThread.getFractionPlayed()));
                String title = meta.title + " (" + time + ")";
                String artist = meta.artist;
                String volume = showVolume ? I18n.format("minetunes.gui.volume", "+" + (int) (MineTunesTwelve.musicPlayerThread.getRelativeVolume() * 100) + "%") : "";
